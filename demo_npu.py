...
def run_demo(model_path, data_dir, duration=20):
    print_banner()
    ...
    if len(image_paths) == 0:
        print("[ERROR] No images found. Run 'download_data.py' first.")
        return
...
if __name__ == "__main__":
    ...
    if not Path(args.model).exists():
        print(f"Model {args.model} not found! Please run 'prepare_model.py' first.")
    else:
        run_demo(args.model, args.data, args.duration)
